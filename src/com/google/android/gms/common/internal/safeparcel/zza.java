...
public class zza
{
  ...
  private static void zza(Parcel paramParcel, int paramInt1, int paramInt2, int paramInt3)
  {
    if (paramInt2 != paramInt3) {
      throw new zza.zza("Expected size " + paramInt3 + " got " + paramInt2 + " (0x" + Integer.toHexString(paramInt2) + ")", paramParcel);
    }
  }
  ...
  public static int zzau(Parcel paramParcel)
  {
    ...
    int i = paramParcel.dataPosition();
    if (zzca(j) != 20293) {
      throw new zza.zza("Expected object header. Got 0x" + Integer.toHexString(j), paramParcel);
    }
    j = i + k;
    if ((j < i) || (j > paramParcel.dataSize())) {
      throw new zza.zza("Size read is invalid start=" + i + " end=" + j, paramParcel);
    }
    return j;
  }
